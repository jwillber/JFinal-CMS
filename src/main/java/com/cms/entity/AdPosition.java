...
public class AdPosition extends BaseAdPosition<AdPosition> {
	...
	public List<AdPosition> findAll(){
		return find("select * from cms_ad_position");
	}
	...
	public Page<AdPosition> findPage(String name,Integer pageNumber,Integer pageSize){
		...
		return paginate(pageNumber, pageSize, "select *", "from cms_ad_position where 1=1 "+filterSql+orderBySql);
	}
    ...
    public List<Ad> getAds(){
        if(ads == null){
            ads = new Ad().dao().find("select * from cms_ad where adPositionId=? order by sort asc,createDate desc",getId());
        }
        return ads;
    }
}
